...
export interface MemoryRecallInput {
  ...
  /** 本轮绑定 Room 的甄选记忆（gateway 注入，已按预算裁好；独立于 charBudget）。 */
 ...
 * 列表，画像与场景目录保留头部。结果为空时返回 null（调用方跳过注入）。
 */
export function formatRecallResult(input: MemoryRecallInput, charBudget: number): string | null {
  const sections: string[] = [];

  let roomSection: string | null = null;
  if (input.roomMemories && input.roomMemories.length > 0) {
    const lines = input.roomMemories.map((item) => {
      const date = item.updatedAt?.slice(0, 10) ?? "";
      const type = item.type ? `（${item.type}）` : "";
      return `- ${item.content}${type}${date ? ` [${date}]` : ""}`;
    });
    roomSection = `[Room 记忆]（用户为当前 Context Room 甄选的记忆，Room 相关问题优先参考）\n${lines.join("\n")}`;
  }
  ...
  if (sections.length === 0 && !roomSection) return null;

  let body = sections.join("\n\n");
  if (body.length > charBudget) {
    body = `${body.slice(0, charBudget)}\n…（已截断，可用 memory_search 查询更多）`;
  }
  const content = roomSection ? (body ? `${roomSection}\n\n${body}` : roomSection) : body;

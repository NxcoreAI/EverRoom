...
> 版本：v1 · 2026-09-02 · 状态：**M1–M3 已实现并真机验证四流；M4 增量迭代已实现（2026-09-02）**
> **M4 实施备注（2026-09-02）**：previousInvocationId 占位契约落地（里程碑表 M4 行）；另随真机验证落地两项修正并入工作区：draft-edit 输出契约删除 fromOffset/toOffset 行内偏移变体（535eaaf）、runBoundary 改为"不要先调用 document_read"（工作区待提交）。配套增补：写作风格新增 review_decision 行为信号（见 writing-style-profile-plan §4.1，同批工作区）。
...
| **M4 增量迭代（previousInvocationId 落地）** ✅ 已实现（2026-09-02，真机验证通过） | document_draft 传 previousInvocationId 时回读**本会话**上一次 doc-writer 产出（授权：doc-writer + primary_agent + parentSessionId 匹配 + completed），归一化注入 input.previousDraft（标题+正文，≤50K 截断；edit hunk 序列化为编号清单）；返回 previousDraftApplied；跨会话/不存在时静默回退普通生成。SKILL 纪律：在上一稿上做增量修改，保留未涉及内容，不从头重写 | "再简洁一点"类跟进指令不再整篇重生成（消除 15:15 那类 42s 重做）；真机四流已验证 | M3（共用 docWriterDraftFromStructuredOutput 归一化） |

...
describe("Nango authorization",()=>{
 ...
 it("creates a provider-scoped connect session and automatically registers the tagged connection",async()=>{const {db,repo}=await setup();const manager=new ConnectorManager(repo,{discoverScopes:async()=>[{id:"me",displayName:"Mailbox"}],async *pull(){}} as any);let postBody:any;let query:any;let attemptId="";const http={post:async(path:string,body:any)=>{postBody=body;attemptId=body.tags.auth_attempt_id;return {data:{data:{token:"short-lived",connect_link:"https://connect.nango.test/?session_token=short-lived",expires_at:new Date(Date.now()+60_000).toISOString()}}};},get:async(path:string,config:any)=>{query=config.params;return {data:{connections:[{connection_id:"nango-created",provider_config_key:"google-mail",tags:{auth_attempt_id:attemptId},errors:[]}]}};}} as any;const authorization=new NangoAuthorizationService("https://nango.test","secret",{gmail:"google-mail",outlook:"microsoft-mail"},manager,http);const started=await authorization.start("gmail");expect(postBody).toMatchObject({allowed_integrations:["google-mail"],tags:{auth_attempt_id:started.id,end_user_id:`everroom-local-${started.id}`}});expect(started.authorizationUrl).toBe("https://connect.nango.test/?session_token=short-lived&apiURL=https%3A%2F%2Fnango.test");const completed=await authorization.status(started.id);expect(query).toEqual({"tags[auth_attempt_id]":started.id,limit:10});expect(completed).toMatchObject({status:"connected",connection:{provider:"gmail",connectionName:"nango-created",service:"google-mail"}});expect(repo.listScopes()).toHaveLength(1);await manager.dispose();db.close();});
 it("reads the latest secret while an authorization is in progress",async()=>{const {db,repo}=await setup();const manager=new ConnectorManager(repo,{discoverScopes:async()=>[{id:"me",displayName:"Mailbox"}],async *pull(){}} as any);let secret="first";let attemptId="";const headers:string[]=[];const http={post:async(_path:string,body:any,config:any)=>{attemptId=body.tags.auth_attempt_id;headers.push(config.headers.Authorization);return {data:{data:{connect_link:"https://connect.nango.test/",expires_at:new Date(Date.now()+60_000).toISOString()}}};},get:async(_path:string,config:any)=>{headers.push(config.headers.Authorization);return {data:{connections:[{connection_id:"connection",provider_config_key:"google-mail",tags:{auth_attempt_id:attemptId},errors:[]}]}};}} as any;const authorization=new NangoAuthorizationService("https://nango.test",()=>secret,{gmail:"google-mail"},manager,http);const started=await authorization.start("gmail");secret="second";await authorization.status(started.id);expect(headers).toEqual(["Bearer first","Bearer second"]);await manager.dispose();db.close();});

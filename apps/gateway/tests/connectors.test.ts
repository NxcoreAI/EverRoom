...
describe("connector manager recovery",()=>{
 ...
 it("fans synced mail upserts (not tombstones) out to the memory sink as markdown",async()=>{const {db,repo}=await setup();const c=repo.registerConnection({provider:"gmail",service:"g",connectionName:"c"});const s=repo.ensureScope(c.id,"me","Mailbox");const executor={async *pull(){yield {changes:[{kind:"upsert",message:{providerMessageId:"m-1",subject:"季度总结",textBody:"正文内容",receivedAt:"2026-08-19T09:00:00Z",addresses:[{role:"from",displayName:"张三",address:"z@e.com"},{role:"to",address:"me@e.com"}]}},{kind:"tombstone",providerMessageId:"m-2"}]};}} as any;const manager=new ConnectorManager(repo,executor);const seen:any[]=[];manager.setMemorySink(async(input)=>{seen.push(input);});const run=manager.trigger(s.id,"full");await manager.dispose();expect(seen).toEqual([{kind:"mail",provider:"gmail",connectionId:c.id,documentId:"m-1",title:"季度总结",markdown:"# 季度总结\n\n发件人：张三 <z@e.com>\n\n收件人：me@e.com\n\n时间：2026-08-19T09:00:00Z\n\n正文内容"}]);expect(repo.getRun(run.id)).toMatchObject({status:"completed"});db.close();});
 it("fans synced calendar upserts out to the memory sink as markdown",async()=>{const {db,repo}=await setup();const c=repo.registerConnection({provider:"google-calendar",service:"gc",connectionName:"c"});const s=repo.ensureScope(c.id,"primary","Primary calendar");const executor={async *pull(){yield {changes:[],calendarChanges:[{kind:"upsert",event:{providerEventId:"ev-1",title:"评审会",startsAt:"2026-08-20T02:00:00Z",endsAt:"2026-08-20T03:00:00Z",timeZone:"Asia/Shanghai",location:"会议室 A",organizer:{role:"organizer",address:"o@e.com"},attendees:[{role:"attendee",address:"a@e.com"}],description:"过方案"}},{kind:"tombstone",providerEventId:"ev-2"}]};}} as any;const manager=new ConnectorManager(repo,executor);const seen:any[]=[];manager.setMemorySink(async(input)=>{seen.push(input);});const run=manager.trigger(s.id,"full");await manager.dispose();expect(seen).toEqual([{kind:"calendar",provider:"google-calendar",connectionId:c.id,documentId:"ev-1",title:"评审会",calendarId:"primary",markdown:"# 评审会\n\n时间：2026-08-20T02:00:00Z → 2026-08-20T03:00:00Z（Asia/Shanghai）\n\n地点：会议室 A\n\n组织者：o@e.com\n\n与会人：a@e.com\n\n## 描述\n\n过方案"}]);expect(repo.getRun(run.id)).toMatchObject({status:"completed"});db.close();});
 it("marks invalid Gmail history for rebuild without clearing its cursor",async()=>{const {db,repo}=await setup();const c=repo.registerConnection({provider:"gmail",service:"g",connectionName:"c"});const s=repo.ensureScope(c.id,"me","Mailbox");db.sqlite.prepare("update sync_scopes set source_cursor='opaque-history' where id=?").run(s.id);const error=Object.assign(new Error("expired"),{response:{status:404}});const executor={async *pull(){throw error;}} as any;const manager=new ConnectorManager(repo,executor);manager.trigger(s.id,"incremental");await manager.dispose();expect(repo.getScope(s.id)).toMatchObject({state:"resync_required",sourceCursor:"opaque-history"});await manager.dispose();db.close();});
 it("resumes a failed full sync from the run cursor and clears it once completed",async()=>{const {db,repo}=await setup();const c=repo.registerConnection({provider:"gmail",service:"g",connectionName:"c"});const s=repo.ensureScope(c.id,"me","Mailbox");const changeA={kind:"upsert" as const,message:{providerMessageId:"m-1",subject:"A",providerRevision:"1"}};const changeB={kind:"upsert" as const,message:{providerMessageId:"m-2",subject:"B",providerRevision:"1"}};const resumeCursor='resume:{"phase":"list","anchor":"9001","pageToken":"t2"}';let failAfter:number|null=1;const seenContinuations:Array<string|null>=[];const executor={async *pull(scope:any){seenContinuations.push(scope.continuation ?? null);if(failAfter===null){yield {changes:[],terminalCursor:"h-1"};return;}let yielded=0;for(const page of [{changes:[changeA],continuation:resumeCursor},{changes:[changeB]}]){if(yielded>=failAfter)throw new Error("Gmail API error: quota exceeded");yield page;yielded+=1;}throw new Error("Gmail API error: quota exceeded");}} as any;const manager=new ConnectorManager(repo,executor);const cursorOf=(runId:string)=>(db.sqlite.prepare("select cursor from sync_runs where id=?").get(runId) as any).cursor;// 首轮：第 1 页成功、第 2 页失败 → 断点落库、processed 保留。
 const run1=manager.trigger(s.id,"full");await manager.dispose();expect(repo.getRun(run1.id)).toMatchObject({status:"failed",processed:1});expect(cursorOf(run1.id)).toBe(resumeCursor);// 续传：executor 收到断点；一页未出 → 断点清空自愈。
 failAfter=0;const run2=manager.trigger(s.id,"full");await manager.dispose();expect(seenContinuations).toEqual([null,resumeCursor]);expect(repo.getRun(run2.id)).toMatchObject({status:"failed",processed:0});expect(cursorOf(run2.id)).toBeNull();// 再次 full：断点已清 → 从第 0 页重来；跑通 → source_cursor 落地、全部断点清除。
 failAfter=null;const run3=manager.trigger(s.id,"full");await manager.dispose();expect(seenContinuations).toEqual([null,resumeCursor,null]);expect(repo.getRun(run3.id)).toMatchObject({status:"completed"});expect(repo.getScope(s.id)).toMatchObject({sourceCursor:"h-1",state:"idle"});expect(repo.latestResumableRun(s.id,run3.id)).toBeNull();db.close();});
 it("rebuild neither resumes from nor writes run cursors",async()=>{const {db,repo}=await setup();const c=repo.registerConnection({provider:"gmail",service:"g",connectionName:"c"});const s=repo.ensureScope(c.id,"me","Mailbox");db.sqlite.prepare("insert into sync_runs(id,scope_id,mode,status,cursor,started_at) values(?,?,?,?,?,?)").run("old-failed",s.id,"full","failed",'resume:{"phase":"list","anchor":"1"}',new Date(0).toISOString());repo.markResyncRequired(s.id);const seen:Array<string|null>=[];const executor={async *pull(scope:any){seen.push(scope.continuation ?? null);yield {changes:[],terminalCursor:"h-2"};}} as any;const manager=new ConnectorManager(repo,executor);const run=manager.trigger(s.id,"rebuild");await manager.dispose();expect(seen).toEqual([null]);expect(repo.getRun(run.id)).toMatchObject({status:"completed"});expect((db.sqlite.prepare("select cursor from sync_runs where id=?").get("old-failed") as any).cursor).toBe('resume:{"phase":"list","anchor":"1"}');db.close();});
 it("purges an orphan connection when scope discovery fails",async()=>{const {db,repo}=await setup();const manager=new ConnectorManager(repo,{discoverScopes:async()=>{throw new Error("discovery failed");},async *pull(){}} as any);await expect(manager.register({provider:"outlook",service:"o",connectionName:"c"})).rejects.toThrow("discovery failed");expect(repo.listConnections()).toEqual([]);db.close();});
});
describe("connector routes",()=>{
 it("inherits bearer authentication and rejects disabled mutations",async()=>{const dir=await mkdtemp(join(tmpdir(),"connector-route-"));dirs.push(dir);const app=await createServer({host:"127.0.0.1",port:0,dataDir:dir,databasePath:join(dir,"gateway.sqlite"),migrationsDir:resolve("drizzle"),runtimeManifestPath:join(dir,"runtime.json"),logLevel:"silent",authToken:"test-token-0123456789",agentRuntime:"fake",memory:null,pi:null,backgroundPi:null,asrInputDir:join(dir,"recordings"),asr:null,knowledge:null,ingestFilter:{enabled:false,mode:"observe",confidenceThreshold:0.7,batchSize:5,batchDelayMs:0,exemptSourceKinds:[],toolsEnabled:false,maxToolCalls:8,rulesFile:"",rulesMaxBytes:2048,insightEnabled:false,insightIntervalMs:3_600_000},cursorCompletionPi:null,mcpConfigPath:join(dir,"agent","mcp.json"),webSearch:null});expect((await app.inject({url:"/v1/nango-connectors/status"})).statusCode).toBe(401);const headers={authorization:"Bearer test-token-0123456789"};expect((await app.inject({url:"/v1/nango-connectors/status",headers})).json()).toMatchObject({enabled:false,connections:[]});expect((await app.inject({method:"POST",url:"/v1/nango-connectors/connections",headers,payload:{provider:"gmail",service:"g",connectionName:"c"}})).statusCode).toBe(503);await app.close();});
});
import {
  connectorEmails,

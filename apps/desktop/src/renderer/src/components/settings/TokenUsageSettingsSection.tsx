      ...
      {error ? <div className="token-usage-empty"><strong>{t('surface:settings.tokenUsageError')}</strong><button type="button" onClick={() => void load()}>{t('surface:settings.retry')}</button></div> : loading ? <div className="token-usage-empty">{t('surface:settings.loading')}</div> : (
        <>
          <div className="token-usage-metrics">
            <div className="token-usage-metric input"><span><Database aria-hidden="true" />{t('surface:settings.inputTokens')}</span><strong>{formatTokens(totals.input)}</strong></div>
            <div className="token-usage-metric output"><span><Zap aria-hidden="true" />{t('surface:settings.outputTokens')}</span><strong>{formatTokens(totals.output)}</strong></div>
            <div className="token-usage-metric cache"><span><Gauge aria-hidden="true" />{t('surface:settings.cacheHitTokens')}</span><strong>{formatTokens(totals.cache)}</strong></div>
          </div>
          {data.length === 0 || max === 0 ? <div className="token-usage-empty">{t('surface:settings.tokenUsageEmpty')}</div> : <div className="token-usage-chart-wrap">
            <div className="token-usage-chart-header"><div><strong>{t('surface:settings.tokenTrend')}</strong>{selected ? <small>{`${selected.label} · ${t('surface:settings.inputTokens')} ${formatTokens(selected.input)} · ${t('surface:settings.outputTokens')} ${formatTokens(selected.output)} · ${t('surface:settings.cacheHitTokens')} ${formatTokens(selected.cache)}`}</small> : null}</div><div className="token-usage-legend" aria-label={t('surface:settings.chartLegend')}><span className="input"><i />{t('surface:settings.inputTokens')}</span><span className="output"><i />{t('surface:settings.outputTokens')}</span><span className="cache"><i />{t('surface:settings.cacheHitTokens')}</span></div></div>

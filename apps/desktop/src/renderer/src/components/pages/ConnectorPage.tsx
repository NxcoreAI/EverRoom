import { AlertTriangle, CalendarDays, Check, ChevronDown, ChevronRight, CirclePause, Database, Eye, FileText, KeyRound, LoaderCircle, Mail, MapPin, Play, Plus, RefreshCw, Trash2, Users, Wrench, X } from 'lucide-react'
...
export function ConnectorSection() {
  ...
  useEffect(() => {
    if (!connectors || authorization?.status !== 'pending') return
    let active = true
    const check = async () => {
      try {
        const next = await connectors.authorizationStatus(authorization.id)
        if (!active) return
  ...
  }, [authorization?.id, authorization?.status, connectors, refresh])
  ...
  const startAuthorization = async (provider: 'gmail' | 'outlook' | 'google-docs' | 'notion' | 'google-calendar') => {
    ...
    setError(null)
    try { setAuthorization(await connectors!.startAuthorization(provider)) }
  ...
  return <section className="connector-debug-page connector-section">
    ...
    {view === 'connections' ? <ConnectionsView connections={connections} scopes={status.scopes} selectedId={selectedConnection} onSelect={setSelectedConnection} busy={busy} onAuthorize={(provider) => void startAuthorization(provider)} onSync={triggerConnection} onDisable={(id) => { if (window.confirm('停用后将停止该连接的自动同步。确认继续？')) void run(`disable:${id}`, () => connectors.disableConnection(id)) }} onPurge={(id) => { if (window.confirm('确认清理该连接器的本地数据？此操作不可撤销。')) void run(`purge:${id}`, () => connectors.purgeConnection(id)) }} /> : null}

    ...
    <>
      <aside className="context-room-workspace-middle">
        <div className="context-room-resource-tree">
          <label><Search aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('contextRoom:fakeDocument.searchDocumentsInThisRoom')} aria-label={t('contextRoom:fakeDocument.searchDocumentsAriaLabel')} /></label>
          <div className="context-room-resource-scroll" role="tree" aria-label={t('contextRoom:fakeDocument.roomResources')}>
      ...
      <section className="context-room-workspace-content">
        <button type="button" className="context-room-mobile-back" onClick={onBackToLibrary}><ChevronLeft aria-hidden="true" />{t('contextRoom:fakeDocument.backToDocuments')}</button>

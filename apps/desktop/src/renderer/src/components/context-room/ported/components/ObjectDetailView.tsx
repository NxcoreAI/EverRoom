...
function ObjectTopbar({
  ...
  return (
    <div className="context-room-object-topbar">
      <button
        type="button"
        aria-label={t('返回 Context Room 详情')}
...
function ExternalObjectState({ kind }: { kind: 'file' | 'mail' | 'meeting' }) {
  ...
  return (
    <section className="context-room-external-state" aria-label={t('外部对象状态')}>
      {[
        '来源服务',
        '账号',
        '同步状态',
...
function FileDetail({
  ...
  return (
    <>
      <ObjectTopbar label="文件" onBack={onBack}>
        <Tag>{file.extension}</Tag>
        <Tag variant="success">{t('已索引')}</Tag>
        <span className="context-room-object-meta">{file.time}</span>
        <button
          type="button"
          className="context-room-ghost context-room-small"
          onClick={() => setNotice(t('已定位来源：{source}', { source: file.source ?? t('本地文件夹') }))}
        >
          {t('打开来源')}
        </button>
        <button
        ...
        <button
          ...
          onClick={() => setNotice(t('已将文件与摘要发送给 Agent'))}
        >
          {t('让 Agent 分析')}
        </button>
      </ObjectTopbar>
      <div className="context-room-object-shell">
        ...
        <Panel title={t('内容预览')} className="context-room-preview-panel">
          <div className="context-room-preview-sections">
            {[['01', t('资料概览'), file.summary]].map(([index, title, text]) => (
      ...
      <ActionConfirmDialog
        ...
        title={t('移至回收站')}
        summary={t('将“{name}”移至回收站。', { name: file.name })}
        rows={[
          { label: t('所属 Room'), value: room.title },
          { label: t('文件位置'), value: file.source ?? t('本地文件夹') },
        ]}
        sources={[{ type: t('文件'), name: file.name }]}
        risk={t('文件将从当前 Room 的活跃资料中移除，可在回收站恢复。')}
        confirmLabel={t('移至回收站')}
        danger
  ...
  return (
    <section className="context-room-task-reference" data-testid="context-room-task-reference">
      ...
      <div className="context-room-object-detail-grid">
        <Panel
          title={t('执行状态')}
          action={<Tag variant={statusVariant(task.status)}>{t(task.status)}</Tag>}
        ...
        >
          <div className="context-room-task-progress-overview">
            ...
            <div className="context-room-progress" aria-label={t('任务进度 {progress}', { progress: progressLabel })}>
          ...
          <div className="context-room-timeline">
            ...
            >
              <div className="context-room-timeline-time">{t('当前')}</div>
              <div className="context-room-task-timeline-copy">
                <b>{t(task.status)}</b>
                <span>{t('任务进度已更新至 {progress}', { progress: progressLabel })}</span>
              </div>
            </div>
            <div className="context-room-timeline-item context-room-timeline-done">
              <div className="context-room-timeline-time">{t('创建时')}</div>
              <div className="context-room-task-timeline-copy">
                <b>{t('任务已建立')}</b>
                <span>{t('来源：{source}', { source: sourceName })}</span>
              </div>
            </div>
          </div>
        </Panel>
        <aside>
          <Panel title={t('来源与上下文')} className="context-room-task-context-panel">
            <span className="context-room-task-context-label">{t('原始来源')}</span>
    ...
    >
      ...
      <header className="context-room-meeting-hero">
        ...
        <div>
          <h1>{meeting.title}</h1>
          <p>
            {meeting.attendees?.join('、') || t('未记录参与人')} · {meeting.duration ?? t('时长未记录')}
          </p>
        </div>
      </header>
      <ExternalObjectState kind="meeting" />
      <div className="context-room-meeting-tabs" role="tablist" aria-label={t('会议详情')}>
        {(
          [
            ['summary', 'AI 纪要'],
      ...
      {tab === 'summary' ? (
        <Panel title={t('AI 纪要')} className="context-room-meeting-panel">
          ...
          <div className="context-room-meeting-facts">
            ...
            <span>
              <small>{t('地点')}</small>
              <b>{meeting.location ?? t('线上会议')}</b>
            </span>
            <span>
              <small>{t('所属 Room')}</small>
              <b>{room.title}</b>
            </span>
          </div>
        </Panel>
        ...
        <section className="context-room-panel context-room-meeting-actions">
          {actions.map((action) => (
            <article key={action.id}>
              <div>
                <b>{action.title}</b>
                <span>{t('负责人 {owner}', { owner: action.owner })}</span>
              </div>
              <button
                type="button"
      ...
      <ActionConfirmDialog
        ...
        title={t('写入任务')}
        summary={t('将把该会议行动项写入当前 Room 的正式任务列表。')}
        rows={[
          {
            label: t('行动项'),
          ...
          {
            label: t('负责人'),
            value: actions.find((action) => action.id === pendingActionId)?.owner ?? t('待补充'),
          },
        ]}
        sources={[{ type: t('会议'), name: meeting.title }]}
        risk={t('写入后会进入任务列表与 Room 活动记录。')}
        confirmLabel={t('确认写入')}
  ...
  return (
    <section className="context-room-mail-reference" data-testid="context-room-mail-reference">
      ...
      <header className="context-room-page-header context-room-object-app-header">
        <div>
          <h1 className="context-room-page-title">{t('邮件')}</h1>
        </div>
        <div className="context-room-page-actions">
          <button
            type="button"
      ...
      <div className="context-room-mail-layout" data-testid="context-room-mail-layout">
        ...
        >
          {visibleMails.length ? (
            visibleMails.map((item) => (
              ...
              >
                <span className="context-room-mail-list-meta">
                  <b>{item.sender}</b>
                  {item.unread ? <Tag variant="info">{t('未读')}</Tag> : null}
                  <time>{item.time}</time>
                </span>
            ...
            <div className="context-room-empty">{t('这里还没有邮件')}</div>
          )}
        </section>
        <section className="context-room-mail-detail">
          <header className="context-room-mail-header">
            ...
            <div className="context-room-mail-detail-actions">
              ...
              >
                <Archive className="size-4" aria-hidden="true" />
                {t(mail.folder === 'archive' ? '移回收件箱' : '归档')}
              </button>
            </div>
          </header>
          <ExternalObjectState kind="mail" />
          <Panel title={t('AI 摘要')} className="context-room-mail-summary">
            <div
          ...
          <Panel
            title={t('回复草案')}
            action={<Tag variant="info">{t('待确认')}</Tag>}
      ...
      <ActionConfirmDialog
        ...
        title={t('加入任务')}
        summary={t('将把邮件中的任务候选加入当前 Room。')}
        rows={[
          { label: t('任务'), value: '补充正式报价函和交付范围' },
          { label: t('截止时间'), value: '07-24' },
        ]}
        sources={[{ type: t('邮件'), name: mail.title }]}
        confirmLabel={t('加入')}
      ...
      <ActionConfirmDialog
        ...
        onOpenChange={(open) => !open && setConfirmAction(null)}
        title={t('发送邮件')}
        summary={t('将向外发送当前回复草案。')}
        rows={[
          { label: t('收件人'), value: '张总 · 星港科技' },
          { label: t('主题'), value: mail.title },
        ]}
        sources={[{ type: t('邮件'), name: mail.title }]}
        risk={t('向外发送邮件属于高风险动作，发送后会进入活动记录。')}
        confirmLabel={t('确认发送')}
...
function MemoryDetail({
  ...
  return (
    <section className="context-room-memory-reference" data-testid="context-room-memory-reference">
      ...
      <header className="context-room-page-header context-room-object-app-header">
        <div>
          <h1 className="context-room-page-title">{t('记忆')}</h1>
        </div>
        <div className="context-room-segmented" aria-label={t('记忆视图')}>
          <button
            type="button"
      ...
      {viewMode === 'graph' ? (
        ...
        <div className="context-room-memory-layout">
          <nav className="context-room-side-tabs context-room-memory-filters" aria-label={t('记忆筛选')}>
            ...
            <div className="context-room-memory-list">
              {visibleMemories.map((item) => (
                ...
                >
                  <b>{item.content}</b>
                  <span>
                    {t(item.type)} · {room.title}
                  </span>
                  <Tag variant={statusVariant(item.status)}>{t(item.status)}</Tag>
                </button>
              ))}
            </div>
          </section>
          <Panel
            ...
            action={<Tag variant={statusVariant(activeMemory.status)}>{t(activeMemory.status)}</Tag>}
            className="context-room-memory-detail-panel"
          >
            <p className="context-room-memory-content">{activeMemory.content}</p>
            <div className="context-room-object-label context-room-memory-source-label">{t('来源')}</div>
            <div className="context-room-source-card" data-icon-tone="ai">
            ...
            <div className="context-room-object-lifecycle">
              ...
              >
                {t(activeMemory.status === '已禁用' ? '已禁用' : '禁用')}
              </button>
            </div>
          </Panel>
        </div>
      ...
      <ActionConfirmDialog
        ...
        rows={[
          { label: t('记忆类型'), value: t(activeMemory.type) },

...
export function ObsidianVaultRoom({ room, vault, onBack, onDisconnect }: {
  ...
  const trashNote = async () => {
    if (!api || !selected || !window.confirm(t(window.nxcore?.platform === 'win32' ? 'surface:obsidian.trashConfirm.windows' : 'surface:obsidian.trashConfirm', { name: selected.resource.name }))) return

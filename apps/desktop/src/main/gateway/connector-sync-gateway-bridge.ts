...
export class ConnectorSyncGatewayBridge {
  constructor(private readonly supervisor: GatewaySupervisor) {}

  status(): Promise<ConnectorSyncStatus> { return this.request('/v1/connectors/sync/status') }
  accounts(): Promise<ConnectorAccount[]> { return this.request('/v1/connectors/accounts') }
  promptProfiles(): Promise<ConnectorPromptProfile[]> { return this.request('/v1/connectors/prompt-profiles') }
  jobs(): Promise<ConnectorSyncJob[]> { return this.request('/v1/connectors/sync/jobs') }
  createJob(input: ConnectorSyncJobInput): Promise<ConnectorSyncJob> {
    return this.request('/v1/connectors/sync/jobs', { method: 'POST', body: JSON.stringify(input) })
  }
  updateJob(id: string, input: Partial<ConnectorSyncJobInput> & { configVersion: number }): Promise<ConnectorSyncJob> {
    return this.request(`/v1/connectors/sync/jobs/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) })
  }
  runJob(id: string): Promise<ConnectorSyncJob> {
    return this.request(`/v1/connectors/sync/jobs/${encodeURIComponent(id)}/run`, { method: 'POST' })
  }
  setJobPaused(id: string, paused: boolean, configVersion: number): Promise<ConnectorSyncJob> {
    return this.request(`/v1/connectors/sync/jobs/${encodeURIComponent(id)}/${paused ? 'pause' : 'resume'}`, {
      method: 'POST', body: JSON.stringify({ configVersion }),
    })
  }
  archiveJob(id: string, configVersion: number): Promise<ConnectorSyncJob> {
    return this.request(`/v1/connectors/sync/jobs/${encodeURIComponent(id)}`, {
      method: 'DELETE', body: JSON.stringify({ configVersion }),
    })
  }
  runs(jobId: string): Promise<ConnectorSyncRun[]> {
    return this.request(`/v1/connectors/sync/jobs/${encodeURIComponent(jobId)}/runs`)
  }
